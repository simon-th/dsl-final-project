...
if __name__=="__main__":
  ...
  parser.add_argument('--evaltest', action='store_true', help='evaluate each test agent against itself and other test agents (may not be used with --benchmark or --vs)', default=False)
  parser.add_argument('--vs', action='store_true', help='evaluate each test agent in set 1 against each agent in set 2 (may not be used with --benchmark or --evaltest)', default=False)
  ...
  elif not args.benchmark and not args.evaltest and not args.vs:
    raise Exception('must set either --benchmark or --evaltest or --vs')
  ...
  if args.benchmark and not args.evaltest:
    ...
    results_df, str_results_df = evaluate_agents(test_agents_1, benchmark_agents, render_mode, args.trials, args.seed)
    print_and_save_results(results_df, str_results_df, args.resultfilename)

  elif args.evaltest and not args.benchmark:
    results_df, str_results_df = evaluate_agents(test_agents_1, test_agents_1, render_mode, args.trials, args.seed)
    print_and_save_results(results_df, str_results_df, args.resultfilename)
  elif args.vs:
    results_df, str_results_df = evaluate_agents(test_agents_1, test_agents_2, render_mode, args.trials, args.seed)
    print_and_save_results(results_df, str_results_df, args.resultfilename)
